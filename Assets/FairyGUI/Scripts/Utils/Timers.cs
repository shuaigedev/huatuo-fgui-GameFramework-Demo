...
namespace FairyGUI
{
    ...
    public delegate Task TimerCallbackAsync(object param);
    ...
    public class TimersAsync
    {
        ...
        Dictionary<TimerCallbackAsync, AnymousAsync_T> _items;
        Dictionary<TimerCallbackAsync, AnymousAsync_T> _toAdd;
        List<AnymousAsync_T> _toRemove;
        List<AnymousAsync_T> _pool;

        TimersAsyncEngine _engine;
        GameObject gameObject;

        private static TimersAsync _inst;
        public static TimersAsync inst
        {
            get
            {
                if (_inst == null)
                    _inst = new TimersAsync();
                return _inst;
            }
        }
        ...
        {
            _inst = this;
            gameObject = new GameObject("[FairyGUI.TimersAsync]");
            //gameObject.hideFlags = HideFlags.HideInHierarchy;
            gameObject.SetActive(true);
            UnityEngine.Object.DontDestroyOnLoad(gameObject);

            _engine = gameObject.AddComponent<TimersAsyncEngine>();

            _items = new Dictionary<TimerCallbackAsync, AnymousAsync_T>();
            _toAdd = new Dictionary<TimerCallbackAsync, AnymousAsync_T>();
            _toRemove = new List<AnymousAsync_T>();
            _pool = new List<AnymousAsync_T>(100);
        }

        public void Add(float interval, int repeat, TimerCallbackAsync callback)
        {
            Add(interval, repeat, callback, null);
        }
         ...
         * @repeat 0 indicate loop infinitely, otherwise the run count
         **/
        public void Add(float interval, int repeat, TimerCallbackAsync callback, object callbackParam)
        {
            if (callback == null)
            {
                Debug.LogWarning("timer callback is null, " + interval + "," + repeat);
        ...
        public void CallLater(TimerCallbackAsync callback)
        {
            Add(0.001f, 1, callback);
        }

        public void CallLater(TimerCallbackAsync callback, object callbackParam)
        {
            Add(0.001f, 1, callback, callbackParam);
        }

        public void AddUpdate(TimerCallbackAsync callback)
        {
            Add(0.001f, 0, callback);
        }

        public void AddUpdate(TimerCallbackAsync callback, object callbackParam)
        {
            Add(0.001f, 0, callback, callbackParam);
        }
        ...
        public bool Exists(TimerCallbackAsync callback)
        {
            if (_toAdd.ContainsKey(callback))
                return true;
        ...
        public void Remove(TimerCallbackAsync callback)
        {
            AnymousAsync_T t;
            if (_toAdd.TryGetValue(callback, out t))
            {
        ...
        private AnymousAsync_T GetFromPool()
        {
            AnymousAsync_T t;
            int cnt = _pool.Count;
            if (cnt > 0)
            {
                ...
                t = new AnymousAsync_T();
            return t;
        }

        private void ReturnToPool(AnymousAsync_T t)
        {
            t.callback = null;
            _pool.Add(t);
        }
        ...
        {
            float dt = Time.unscaledDeltaTime;
            Dictionary<TimerCallbackAsync, AnymousAsync_T>.Enumerator iter;
            ...
            if (_items.Count > 0)
            {
                ...
                while (true)
                {
                    ...
                    AnymousAsync_T i = iter.Current.Value;
                    if (i.deleted)
                    {
                        _toRemove.Add(i);
                        continue;
    ...
    class AnymousAsync_T
    {
        public float interval;
        public int repeat;
        public TimerCallbackAsync callback;
        public object param;

        public float elapsed;
        public bool deleted;

        public void set(float interval, int repeat, TimerCallbackAsync callback, object param)
        {
            this.interval = interval;
            this.repeat = repeat;
            this.callback = callback;
    ...
    class TimersAsyncEngine : MonoBehaviour
    {
        async Task Update()
        {
            try
            {
                await TimersAsync.inst.Update();
            }
            catch(Exception ex)
            {
                Debug.LogError(ex);
